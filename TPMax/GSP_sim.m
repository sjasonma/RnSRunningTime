function [max_i, max_x, n, tot_time, k_left] = GSP_sim(mu, sigma, alpha, delta, n0, bsize, X0)
tic
k = length(mu);
...
%      [r, k_left, sum(n)]
    ...
    sum_i(idx) = sum_i(idx) + bs(idx) .* mu(idx)' + randn(k_left, 1) .* sigma(idx)' .* sqrt(bs(idx));
    n(idx) = n(idx) + bs(idx);
...
sum_i(idx) = sum_i(idx) + (n_add .* mu(idx)' + randn(k_left, 1) .* sigma(idx)' .* sqrt(n_add));
n(idx) = n(idx) + n_add;
